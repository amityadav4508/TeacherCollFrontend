...
const ManageAssignment = () => {
  ...
  // const [idData1, setIdData1] = useState(false)
  ...
  const [param, setParams] = useState(Status.split('/')[3])
  let myArray = Status.split('/')[3]
  console.log(Status, 'gggfgf')
  ...
  const columns = [
    {
      name: 'ORDER ID',
      selector: (row) => row.order_id || <span className="subscribed">Subscribed</span>,
      sortable: true,
    },
    {
      name: 'ASSIGNMENT ID',
    ...
    },
    {
      name: 'CREATED ON',
      selector: (row) =>
        moment(new Date(row.created_at + ' UTC').toString()).format('YYYY-MM-DD hh:mm:a') || 'N/A',
      sortable: false,
    },
    {
      name: 'TITLE',
      selector: (row) =>
        (
          <p
            className="overflow-hidden text-truncate mt-3"
            style={{ width: '120px' }}
            title={row.question}
          >
            {row.question}
...

    {
      name: 'ANSWERED BY',
      selector: (row) =>
        row.teacher_email == null || row.teacher_email == 0
          ? 'pending'
          : row.teacher_email || 'Pending',
      sortable: false,
    },
    {
      name: 'DUE BY',
      selector: (row) =>
        moment(new Date(row.due_date + ' UTC').toString()).format('YYYY-MM-DD hh:mm:a') || 'N/A',
      sortable: false,
    },
    {
      name: 'Rating',
      selector: (row) =>
        <ReactStars count={5} value={row?.rating} edit={false} size={24} activeColor="#ffd700" /> ||
        'N/A',
...

    {
      ...
      selector: (row) =>
        (
          <span
            onClick={() => {
              navigate(`/viewassignment/${row?.id}`)
            }}
          >
  ...
  const handleFilterData = (e) => {
    console.log(e.target.value, e.value,"eeeee")
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      // if (idData1 == true && marked && marked.id.length != 0) {
      //   await dispatch(assignmentStatusAsync(marked))
          ...
          <Card className="Recent-Users card-bt mt-3">
            <Card.Header style={{ backgroundColor: '#fff' }}>
              <div className=" row justify-content-between align-items-center">
                ...
                <Col md={4} xl={3}>
                  <Form.Select aria-label="Default select example" value={action} onChange={(e)=> setAction(e.target.value)}>
                ...
                <Col md={4} xl={3} className="mt-3 mt-lg-0">
                  <Form.Control
