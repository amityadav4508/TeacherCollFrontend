...
import {  forgetPassUserAsync } from 'src/store/features/ForgetPasswordSlice'
...
const ForgetPassUser = () => {
  ...
  useEffect(() => {
    ...
    if (submit && Object.keys(err).length === 0) {
      dispatch(forgetPassUserAsync(emailData))
    }
  }, [submit, err])
                  ...
                  <CForm>
                    ...
                    <CRow>
                      <div className="clm-btn">
                        ...
                        <Link to="/login" className="text-login  h6 text-primary">
                           Go Back Login
                        </Link>
                      </div>
                    </CRow>
