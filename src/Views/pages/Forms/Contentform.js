...
import {getSinglecontentIdAsync} from '../../../store/features/ContentListSlice'
import BackButton from 'src/views/widgets/BackButton'

function ContantForm() {
  let { id } = useParams()
  const dispatch = useDispatch()
  const {contentId} = useSelector((state) => state?.singleContentId)
  ...
  useEffect(() => {
    dispatch(getSinglecontentIdAsync(id))
  }, [id])

  useEffect(() => {
    if (contentId) {
      setName(contentId?.name)
      setEmail(contentId?.email)
      setGender(contentId?.gender)
      setContact(contentId?.contact)
      setQualification(contentId?.qualification)
      setAge(contentId?.age)
      setUser_type(contentId?.user_type)
      setCity(contentId?.city)
      setCountry(contentId?.country)
      setState(contentId?.state)
    }
  }, [contentId])
  ...
  return (
    <>
      ...
      <BackButton/>
        ...
        <h4 className='d-flex justify-content-center'>User Details</h4>
          <Container fluid>
            <Row>
              <Col md="6">
